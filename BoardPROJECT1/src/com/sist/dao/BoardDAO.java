...
public class BoardDAO {
	...
	public BoardVO boardDetailData(int no) // 사용자가 게시물 번호를 주면 게시물 확인 가능
	{
		BoardVO vo = new BoardVO();
		try {
			getConnection(); // UPDATE ==>조회수 증가해야하기 때문에!! => 증가된 데이터를 가져오기!=
								// 클릭하면 조회수 증가!
			// 글삭제도 SELECT로 비번 먼저 확인하고 삭제 가능하게 하기!
			// 한가지 기능을 위해 여러가지 쿼리문장 작성 가능! 유의점! 오토커밋!!
			// 411,412 페이지 - 트랜젝션 프로그램! - 모든 문장 성공수행시- 커밋 // 하나라도 실패시
			// ROLLBACK처리!
			//
	...
	// 데이터를 가져오는 것 은 같다 => 조회수 증가한 부분을 지움!그 외에는 거의 동일 regdate,와 hit, pwd는 가져올 필요
	// x!!!!!!!!!!!!!
	// 1. 2.
	public BoardVO boardUpdateData(int no) // 사용자가 게시물 번호를 주면 게시물 확인 가능
	{
		...
		try {
			...
			String sql = "SELECT no,name,subject,content " + "FROM board " + "WHERE no=?";
